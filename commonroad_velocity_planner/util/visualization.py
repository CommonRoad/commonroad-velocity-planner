...
def visualize_route_with_velocity(
    ...
    v_min: float = np.min(velocity_array)
    v_max: float = np.max(velocity_array)
    ...
    for idx, position in enumerate(route.reference_path):
        ...
        normalized_velocity: float = (velocity_array[idx] - v_min) / (v_max - v_min)
